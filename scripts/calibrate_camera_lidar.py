...
def extract_points_2D(img_msg, now, rectify=False):
    ...
    save_data(
        img,
        "image_color%s-%d.jpg" % (rect, now.secs),
        os.path.join(CALIB_PATH, "images"),
        True,
    )
...
def convert_point2D_to_numpy(msg):
    # Log PID
...
def create_scatter_3D(points, now):
    ...
    return fig, ax, mid_x, mid_y, mid_z


def extract_points_3D(velodyne, now):

    points = convert_point2D_to_numpy(velodyne)
    ...
    fig, ax, mid_x, mid_y, mid_z = create_scatter_3D(points, now)
    ...
    def onpick(event):
        ...
        if picked and (
            x[ind] == picked[-1][0]
            and y[ind] == picked[-1][1]
            and z[ind] == picked[-1][2]
        ):
            return
    ...
    def mousewheel_move(event):
        ...
        ax.set_zlim(
            [mid_z - cur_zrange * scale_factor, mid_z + cur_zrange * scale_factor]
        )
...
def show_points_3D(velodyne, now, corners):
    ...
    if len(corners) > 1:
        ...
        for i in range(1, len(corners)):
            ...
            corner0 = corner1
        # plot last corner with first corner
        ax.plot(
            [corner0[0], corners[0][0]],
            [corner0[1], corners[0][1]],
            [corner0[2], corners[0][2]],
        )
        ax.figure.canvas.draw_idle()
...
def calibrate(points2D=None, points3D=None):
    ...
    success, rotation_vector, translation_vector, inliers = cv2.solvePnPRansac(
        ...
        iterationsCount=int(1e6),
    )
    if not success:
        rospy.logwarn("PnP RANSAC Failed")
        # return

    # Compute re-projection error.
    points2D_reproj = cv2.projectPoints(
        points3D, rotation_vector, translation_vector, camera_matrix, dist_coeffs
    )[0].squeeze()
    assert points2D_reproj.shape == points2D.shape
    error = (points2D_reproj - points2D)[
        inliers
    ].squeeze()  # Compute error only over inliers.
    print("Reprojection error shape: ", error.shape)
    ...
    # Save extrinsics
    np.savez(
        os.path.join(folder, "extrinsics.npz"),
        euler=euler,
        R=rotation_matrix,
...
def project_point_cloud(velodyne, img_msg, image_pub):
    ...
    try:
        transform = TF_BUFFER.lookup_transform("world", "PandarXT-32", rospy.Time())
...
Outputs: None
"""
global IMAGE_MSG, POINTCLOUD_MSG
IMAGE_MSG = None
POINTCLOUD_MSG = None
global camera_info
camera_info = None
...
def rviz_click_callback(msg):
    global rviz_clicks
    point = (msg.point.x, msg.point.y, msg.point.z)
    rviz_clicks.append(point)
    print(rviz_clicks)


# def callback(image, camera_info, velodyne, image_pub=None):
def callback(image, velodyne, image_pub=None, points_pub=None):
    global CAMERA_MODEL, FIRST_TIME, PAUSE, TF_BUFFER, TF_LISTENER
    global IMAGE_MSG, POINTCLOUD_MSG

    IMAGE_MSG = image
    POINTCLOUD_MSG = velodyne

    points_pub.publish(velodyne)
    ...
    if camera_info is not None and FIRST_TIME:
        FIRST_TIME = False
...
def listener(camera_info, image_color, velodyne_points, camera_lidar=None):
    ...
    info_sub = rospy.Subscriber(
        camera_info, CameraInfo, camera_info_callback, queue_size=1
    )
    rviz_sub = rospy.Subscriber(
        "/clicked_point", PointStamped, rviz_click_callback, queue_size=1
    )
    ...
    points_pub = rospy.Publisher(
        "/calibration/current_points", PointCloud2, queue_size=5
    )

    # Synchronize the topics by time
    ats = message_filters.ApproximateTimeSynchronizer(
        [image_sub, velodyne_sub],
        queue_size=15,
        slop=0.1
    ...
    while 1:
        time.sleep(0.1)
        if PAUSE and IMAGE_MSG and POINTCLOUD_MSG:
            now = rospy.get_rostime()

            extract_points_2D(IMAGE_MSG, now)
            while len(rviz_clicks) < 4:
                time.sleep(0.5)
            corners = rviz_clicks
            print(corners)
            show_points_3D(POINTCLOUD_MSG, now, corners)
            # extract_points_3D(POINTCLOUD_MSG, now)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "--calibrate":
        ...
        image_color = "/video_mapping/back"
        image_color = "/camera/color/image_raw"
        camera_info = "/camera/color/camera_info"
        velodyne_points = "/hesai/pandar"
        camera_lidar = None
        PROJECT_MODE = False
